...
def generate_video_with_webhook(
    ...
    Raises:
        ValueError: If webhook_base_url is not configured
    """
    # Get webhook base URL from config
    webhook_base_url = config.replicate.get("webhook_base_url", "")
    if not webhook_base_url:
        raise ValueError(
            "Replicate webhook_base_url not configured. "
            "Please set replicate.webhook_base_url in config.toml or REPLICATE_WEBHOOK_BASE_URL environment variable. "
    ...
    try:
        ...
        webhook_result_manager.remove_result(callback_id)

        if not webhook_result:
            logger.error(f"Webhook timeout for callback_id: {callback_id}")
            return None

        # Extract video URL from webhook result
        status = webhook_result.get("status")

        if status == "succeeded":
            output = webhook_result.get("output")
            if output:
                video_url = output[0] if isinstance(output, list) else output
                logger.success(f"Video generation succeeded: {video_url}")
                return video_url
            else:
                logger.error("Video generation succeeded but no output URL found")
                return None
        elif status == "failed":
